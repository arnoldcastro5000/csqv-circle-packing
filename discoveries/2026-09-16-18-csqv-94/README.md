# Discovery: CSQV N=94 circle packing (beats best-known, exact-LP radii)

An improved packing for the Packomania CSQV problem (variable-radii circles in a
unit square, maximize the sum of radii) at N = 94. It beats the live best-known.
...
| field | value |
|---|---|
| result files | `csqv94.pck` (15 dp, strictly feasible, SEALED at its jammed value), `n94-sealed.txt` (the jammed centers, the `.pck` source), `n94-best.txt` (the pre-seal worker centers) |
| centers found by | the native C++ cold multi-start worker (`cpp/csqv/worker.cpp`), base_seed 0, thread seed 0, no LLM |
| worker mode | `--count` (basin-dedup OFF). In the N=94 spread-vs-count A/B, `--count` beat `--spread` on the champion value (spread led on distinct basins 53894 vs 51537, but count found the better basin). |
| radii + `.pck` by | the exact-LP radii + rounding-aware feasibility shrink (the parse-back safety loop) |
| terminal squeeze | SEALED 2026-09-16: the first-order-squeezed champion was NOT jammed (Donev margin 8.90), so the jamming optimizer (`jam_slp`, a trust-region sequential-LP flex ascent) drove it to its same-basin jammed optimum (margin 0), then the `.pck` was emitted + zero-tol validated |
...
| quantity | value |
|---|---|
| sum of radii (SEALED, re-parsed from the 15-dp `.pck`) | 5.100695331104855 |
| sum before sealing (first-order squeezed) | 5.100678791842069 |
| sealing gain | +1.65e-5 (the champion was un-jammed) |
| Packomania live best-known (fetched 2026-09-16) | 5.098748716187 |
| margin over the live record | +0.001946614918 (about +0.03818%) |

The live best-known was fetched 2026-09-16 from `csqv/csqv.html` (page updated
16-Sep-2026): N=94 = 5.098748716187, credited to Everett Dutton [10] (Gurobi
...
- 94 circles, radii in [0.032969, 0.067791], sorted by increasing radius.
- Containment (centered side-1 square, frame center 0,0): worst wall violation
  -1.0e-12 (inside, robust cushion).
- Non-overlap (all 4371 pairs): worst pair overlap -2.001e-12 (no overlap, robust
  cushion).
- The first line (largest radius) equals the largest radius in the rows.
- STRICTLY FEASIBLE at zero tolerance: YES.

Reproduce with `python3 tools/validate_pck.py discoveries/2026-09-16-18-csqv-94/csqv94.pck`.
...
```
csqv_worker 94 <budget_s> 0 <threads> <out_dir> "Arnold Castro" 15 --record 5.098748716187
```
...
It resumes from `<out_dir>/n94-s<base_seed>-best.txt`, writes the champion centers
to `<out_dir>/n94-best.txt`, and emits `<out_dir>/csqv94.pck` at 15 dp. The worker
