...
The CSQV genome contract (ticket 01) fixed exactly two trusted primitives on `ctx`, `radii_lp`
and `polish`, kept out of the genome and the scoring path. We add a third, `defect_move`, a
...
Locatelli, Schoen) and appeared in Discovery Loop's evolved solvers (arXiv:2609.05093).

## The trade-off

We chose a FIXED, trusted primitive over evolvable genome-shell code. A primitive is ungameable
...
outweigh contract minimalism here, consistent with the repo's quality-first mandate. The genome
still orchestrates WHEN and how often to call it.
